...
- **Design goal:** non-generic, restrained, typographic UI (system fonts, one
  accent, no gradients/glow/emoji). Built by hand, not generated.
- **Currently:** advanced working build — session-local, streaming, settings.
...
| Storage | **none, session-only** (`sessionStorage`, survives refresh, cleared on tab close) | PostgreSQL/Qdrant/Neo4j/Redis |
...
- Markdown-lite renderer: fenced code blocks, inline code, bold/italic, clickable
  links — no HTML injection (everything is escaped first).
...
Kept deliberately small; no single item depends on the MemOS backend.
...
> Persistence stays out of scope by design: one tab, one session. If persistence
> is ever wanted it will be an opt-in local file, never the MemOS backend stores.
